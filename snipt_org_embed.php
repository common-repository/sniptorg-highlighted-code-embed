<?php
/*
	Plugin Name: Highlighted Code Snippet Embed
	Plugin URI: http://snipt.org
	Description: Allows embeding of code saved on snipt.org 
	Version: 1.1
	Author: Dan Wasyluk

  	Copyright 2009 Dan Wasyluk / Snipt.org  (dan@thecollegeblognetwork.com)
...
function fetch_snipt($key) {
	return load('http://snipt.org/embed/widget_rss.php?snipt='. $key);
}
...
function code_replace($content) {
	$regex = '/\[embed_snipt:(.*?)]/i';
	preg_match_all( $regex, $content, $matches );
	
	for($i = 0;$i < count($matches[0]);$i++) {
		$key = $matches[1][$i];
		$url = '<script type="text/javascript" src="http://embed.snipt.org/'. $key .'"></script>';
		
		
		$raw_html = fetch_snipt($key);
		$url .= '<noscript>'. $raw_html .'<br>[code snippet: <a href="http://snipt.org/'. $key .'">http://snipt.org/'. $key .'</a>]</noscript>';
...
add_filter('the_content','code_replace');
